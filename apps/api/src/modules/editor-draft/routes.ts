...
export async function registerEditorDraftModule(
  app: FastifyInstance,
  context: EditorDraftModuleContext,
) {
  ...
  const resolve = async (key: string) =>
    context.resolveProblemId ? await context.resolveProblemId(key) : key;
  const readParams = (request: FastifyRequest) => {
    ...
    return {
      problemKey: params.problemId ?? params.idOrSlug ?? '',
      language: params.language ?? query.language ?? '',
    };
  };
  ...
  const save = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (cause instanceof EditorDraftValidationError)
        return error(reply, request, 400, 'VALIDATION_ERROR', cause.message);
      if (cause instanceof EditorDraftConflictError)
        return error(
          reply,
          request,
          409,
          'DRAFT_CONFLICT',
          cause.message,
          cause.current ? { current: cause.current } : undefined,
        );
      if (cause instanceof Error && cause.message === '23503')
        return error(reply, request, 404, 'NOT_FOUND', 'Problem not found');
      throw cause;
    }
  };

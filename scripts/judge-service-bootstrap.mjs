...
try {
  ...
  if (!existingRole.rowCount)
    await admin.query(`CREATE ROLE ${role} LOGIN PASSWORD ${quoteLiteral(rolePassword)} NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT`);
  else
    await admin.query(`ALTER ROLE ${role} LOGIN PASSWORD ${quoteLiteral(rolePassword)} NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT`);
  ...
  if (!existingDatabase.rowCount)
    await admin.query(`CREATE DATABASE ${database} OWNER ${quoteIdentifier((await admin.query('SELECT current_user')).rows[0].current_user)}`);
...
try {
  ...
  await target.query(`ALTER DEFAULT PRIVILEGES FOR ROLE CURRENT_USER IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ${role}`);
  await target.query(`ALTER DEFAULT PRIVILEGES FOR ROLE CURRENT_USER IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO ${role}`);
  console.log(JSON.stringify({ judgeDatabase: databaseName, judgeRole: roleName, status: 'BOOTSTRAPPED' }));
} finally {
  await target.end();
}

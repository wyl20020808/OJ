...
$Config = @{
  ...
  CompilerRootfsIdentity = 'ffb494c1c8ddf5cbf9357e887abb12adc37c3308016bbfeada9998c3e732c9c5'; CompilerRootfsVersion = 'g++-13 (Ubuntu 13.3.0-6ubuntu2~24.04.1) 13.3.0'; WorkerBinary = ''
...
function Start-Supervisor($state) {
  if (-not $Config.CompilerRootfsIdentity) { throw 'Supervisor prerequisite missing: CompilerRootfsIdentity in config/dev-runtime.local.ps1.' }
...
function Invoke-Doctor {
  ...
  $rootfs=$Config.CompilerRootfsLinuxPath;$rootfsIdentity='';$rootfsVersion='';try{$rootfsIdentity=(Invoke-Wsl @('-d',$Config.WslDistro,'--','cat',"$rootfs.identity") 10000).Trim();$rootfsVersion=(Invoke-Wsl @('-d',$Config.WslDistro,'--','cat',"$rootfs.compiler-version.txt") 10000).Trim()}catch{};Add-Check 'supervisor-binary' (Test-WslFile $Config.SupervisorLinuxBinary -Executable) $Config.SupervisorLinuxBinary;Add-Check 'trusted-probe' (Test-WslFile $Config.SupervisorProbeLinuxPath -Executable) $Config.SupervisorProbeLinuxPath;Add-Check 'compiler-rootfs' (Test-WslFile $rootfs -Directory) $rootfs;Add-Check 'compiler-rootfs-identity' ($rootfsIdentity -eq $Config.CompilerRootfsIdentity -and $Config.CompilerRootfsIdentity) "$rootfsIdentity expected=$($Config.CompilerRootfsIdentity)";Add-Check 'compiler-version' (($rootfsVersion -split "`r?`n")[0] -eq $Config.CompilerRootfsVersion -and $Config.CompilerRootfsVersion) "$rootfsVersion expected=$($Config.CompilerRootfsVersion)";Add-Check 'canonical-origin' ($Config.WebOrigin -eq "http://127.0.0.1:$($Config.WebPort)") $Config.WebOrigin

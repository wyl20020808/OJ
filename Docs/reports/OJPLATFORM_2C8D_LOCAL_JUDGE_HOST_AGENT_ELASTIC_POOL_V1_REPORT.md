...
- Judge Admin Product E2E: PASS using the existing server-configured `OJPLATFORM_PHASE2B_REAL_RUNTIME` operator fixture. A real Product API, Vite preview and Chromium verified authenticated unavailable-state handling, CSRF session behavior, no browser-direct Judge request, and no horizontal overflow at 1440x900, 1024x768 and 390x844. This is not an interaction qualification of the configured Host Agent controls.
...
This evidence verifies Host Agent lifecycle, template capacity accounting, autoscaler decision/action flow, scheduler A/B routing, scale-down ordering and stale-incarnation rejection for the Safe Fixture path. It does not qualify Supervisor/C++ execution, Host Agent restart ownership recovery, configured Host Agent browser interaction, or production readiness. The in-app browser connector itself also could not initialize because its local runtime asset path was unavailable; the Product unavailable-state browser E2E passed through Playwright. No mock node is claimed as runtime evidence.

...
`PASS / LOCAL_SINGLE_HOST_RUNTIME_QUALIFIED`
...
- Host-owned Workers are detached from the Agent process group and unreferenced after the trusted spawn commit. A Host Agent process restart therefore cannot terminate a live Worker merely by exiting; the restarted Agent still treats the persisted PID as untrusted and blocks allocation rather than adopting or killing it.
...
- Fresh real C++20 Host Agent path: PASS. An isolated Judge DB/prefix and a non-root WSL `oj-sandbox` Supervisor (UID/GID 1000) with current-source 2C.4 execution-set endpoints and verified immutable compiler rootfs were used. Host Agent started Node A (`269aa727-252f-4aca-b56d-d0134b16a564`) and Node B (`27e8b0f5-cd1e-466e-b73a-8329424313a9`); both registered `ONLINE` with `REAL_SANDBOXED_EXECUTION`. Scheduler job `81e51a29-225a-40d5-9f44-04015b46b84e` completed AC on A. After A was drained/offlined at zero active jobs, scheduler job `9d87c3d0-4f05-4cd0-b7c5-5f3072c234ef` completed AC on B (`602ccd7a-7f82-4a68-a1f0-fb01973b7b2a`). The real Supervisor record reported `PIPELINE_COMPLETED`, finite cgroup memory/PID limits, and cleanup verified.
- Fresh Host Agent restart safety: PASS. A pre-fix Windows process test exposed Agent parent termination also terminating its non-detached Worker; the Host Agent now starts trusted Workers detached/unreferenced. With Node B incarnation `2b3a7ee0-1989-4d7c-8077-4696cc087018`, force-terminating only the Host Agent PID left Worker PID `26864` alive and its Judge heartbeat advanced. A replacement Agent read persisted ownership, reported `unreconciledNodes: 1`, zero available CPU/RAM, no adopted owned node and a failed reconciliation record; it rejected duplicate start with `409 NODE_OWNERSHIP_RECONCILIATION_REQUIRED`. It did not kill or adopt the Worker.
...
This evidence verifies Host Agent lifecycle, template capacity accounting, autoscaler decision/action flow, scheduler A/B routing, scale-down ordering, stale-incarnation rejection, real C++20 Worker/Supervisor routing, fail-closed Host Agent restart ownership handling, and configured Product browser controls. The initial C++20 attempt correctly surfaced `INFRA_FAILED` because the pre-existing WSL Supervisor binary lacked the current 2C.4 execution-set endpoints; it was replaced only with a current-source temporary binary before the credited A/B evidence. The in-app browser connector itself could not initialize because its local runtime asset path was unavailable; local Chrome was used for browser qualification. No mock node is claimed as runtime evidence.

Host Agent operation history remains bounded process-local observability. A restarted Agent deliberately retains live persisted ownership as an unreconciled block rather than adopting or killing it; the runtime evidence above qualifies that fail-closed behavior. It does not provide automatic post-restart ownership takeover.

## Security and architecture

Browser does not receive Judge/Host credentials. Add Node accepts trusted template IDs only. Judge Workers retain the standalone Judge boundary and do not access Application PostgreSQL. This is a local single-host qualification only; production readiness, multi-host, HA, Kubernetes and cloud provisioning are not claimed.

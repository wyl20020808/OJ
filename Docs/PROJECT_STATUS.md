...
Product Experience Wave 4F Profile Media & Save Experience V2 (2026-09-09): PARTIAL for implemented and automated scope. Fixed the profile PATCH contract root cause, added field-level validation projection and optional-field clearing, reused the shared S3/MinIO client for authenticated multipart avatar/background upload, controlled public media delivery, replacement/removal endpoints, and redesigned Profile/Edit Profile media surfaces. Root/API/Web typechecks, focused Profile/Web 37/37 tests, changed-file lint, and diff checks pass. TEMP media migration numbering remains pending integration; PostgreSQL/MinIO runtime qualification and manual visual acceptance are not verified. See `Docs/reports/OJPLATFORM_WAVE4_PROFILE_MEDIA_SAVE_EXPERIENCE_V2_REPORT.md`.

...
OJPlatform AI Host Integration Final Merge + Canonical Runtime Sync V1 (2026-09-24): PASS / MERGED. Canonical `main` at `738f061838a06c6ce558c9cbbf3271d22ee788b3` merged qualified feature tip `a15923cb4cbd8ea692167fc79c5c9310c135f44d` using normal `--no-ff` merge `a5fe3d7180bed342d7c7287e42aa22f9aee7bfdc`; follow-up format/lint corrections are on `c3391b56cab1e5e69cb92ee05259db649d52df13`. Product migrations `0037_ai_capability_usage` and `0038_ai_capability_usage_prompt_provenance` are tracked and were already present in the shared DB ledger before canonical start; migration status is `UP_TO_DATE` (41 applied, 0 unknown, 0 pending, 0 checksum mismatch). Canonical Runtime Manager start, restart, warm start, and service readiness pass. `pnpm ci:check` passes with 1144 tests collected, 1091 passed, 48 unchanged known failures, 5 unchanged pending, and no missing tests/new failures/signature changes/new pending. AI integration 66/66; OnlineCodeEditor 56/56 plus typecheck; AIBridge `npm run verify` 509/509. Business counts users/problems/submissions stayed 155/135/69395 through canonical startup. The AI-ledger integration test added 5 request and 1 attempt rows during full qualification (25/5 -> 30/6); no migration ran during this task. E2E baseline was not run because its shared-database fixture deletes all users/problems/submissions. No remote push; no AIBridge or AlgoQuest source changes. Full evidence and preserved workspace inventory: `Docs/reports/OJPLATFORM_AI_HOST_INTEGRATION_MERGE_V1_REPORT.md`.

OJPlatform Full-Stack Synchronization V1 GitHub CI Qualification (2026-09-22): PASS / GREEN. `OJPlatform CI` run `35726474650` at `a6b57323906213d5b760cab006146f2da12dbf8e` passed 18/18 steps on `ubuntu-latest` (main == origin/main). The inherited workflow was red before this goal; each failure was diagnosed from the authenticated job log and repaired in one scoped commit: exact serialized Vitest known-failure gate (`55d2220`), canonical `quay.io/minio/minio` (`96e7c92`), product migration `DATABASE_URL` (`65aa9c8`), serialized qualification (`aabfd65`), portable Vitest alias resolving the pinned OnlineCodeEditor submodule instead of a Windows worktree plus a real pinned-editor accessibility assertion (`769f587`), stale profile-capability integration contract aligned to the current heatmap/wrong-book truth (`fa088f2`), portable Playwright browser selection (`PLAYWRIGHT_EXECUTABLE_PATH` override; Playwright-managed Chromium by default) (`2897c9b`), two nondeterministic unit fixtures made deterministic and requalified 20/20 (`d973869`), exact Playwright known-failure gate (`c3b0b19`), and JSON-report identity normalization (`a6b5732`). Final gates: `ci:check` 1082 collected / 1030 passed / 47 known failures / 1 resolved with `new=0`, `signature changes=0`, `new pending=0`; `integration` 20/20; `test:e2e:baseline` 31 identities = 1 passed (composed Full-Stack Synchronization journey) / 17 known failures / 13 known gated skips with exact signature comparison. No test was deleted or newly skipped; both gates run complete suites and fail on missing identities, new failures, new skips, changed signatures, report errors, or reduced collection. TECH DEBT: 17 legacy E2E failures predate this goal (stale selectors/DOM contracts, legacy English UI expectations after Chinese productization, test-owned `DELETE FROM problems` cleanup colliding with contest foreign keys) and are pinned by `Docs/reports/artifacts/fullstack-sync-v1/e2e-baseline.json` until repaired in a dedicated goal. Manual visual acceptance remains PENDING USER. Deferred product contracts unchanged: contest scoring/standings, verdict-derived wrong-book aggregation, atomic Contest submission binding/dispatch. See `Docs/reports/OJPLATFORM_FULL_STACK_SYNCHRONIZATION_V1_REPORT.md`.

OJPlatform Full-Stack Synchronization V1 Integration (2026-09-22): PASS / MERGED. Fresh candidate `codex/fullstack-sync-v1-integration-v1` started from fetched `origin/main` `ed192ff411b62337fc5071c3f59cef70d0388f2c` and merged feature tip `547782e2cb4b591ecc9d7e4900d61bbd57bfc771` with normal `--no-ff` merge `c71be34a`; no conflicts occurred. Integration found and fixed one timing-dependent redundant Problem Library reload (`c05ed7a`): tag-catalog readiness no longer reloads a list unless a category filter depends on the catalog, with deterministic regression coverage. Final full Vitest is 1082 total / 1029 passed / 48 failed / 5 pending and matches baseline by exact failure identity and signature (`new=0`, `resolved=0`, `signature changes=0`). Repository format/lint/typecheck, architecture, root/API/Web builds, focused regression, diff check, pinned OnlineCodeEditor 56/56 tests/build, managed runtime start/Doctor, explicit-source `MIXED SOURCE = False` with worker ONLINE, and installed-Chrome composed journey 1/1 pass. Manual visual acceptance remains PENDING USER; standings, wrong-book, and atomic contest-submission binding remain explicit deferred dependencies. See `Docs/reports/OJPLATFORM_FULL_STACK_SYNCHRONIZATION_V1_REPORT.md`.

OJPlatform Full-Stack Synchronization V1 (2026-09-22): PASS / READY FOR INTEGRATION on `codex/fullstack-sync-v1`, based on live main `ed192ff411b62337fc5071c3f59cef70d0388f2c` and qualified feature tip `fbc1e914d4782888f29d2600f4e04b92000baa1e`. The composed Web now maps visible routes/actions to authoritative Auth, Problem, Profile, Submission, Contest, Discussion, Team, Assignment, Social, Sandbox, and Judge contracts; persisted Assignment APIs replace the fabricated Homework dashboard; authentication bootstrap, logout, favorite, failure, and capability states no longer infer success; Judge capabilities distinguish `canView` and `canManage`; and fabricated rankings, wrong-book results, Problem filters, Discussion/Team projections, counts, and inert controls were removed or made explicitly unavailable. Contest standings still report `SCORING_ENGINE_NOT_INTEGRATED`; contest submission creation remains deferred until an atomic binding/outbox design prevents orphaned Judge jobs; wrong-book awaits an authoritative verdict aggregation contract. Full Vitest added 16 passing tests while preserving all 48 baseline failure identities and signatures exactly (`new=0`, `resolved=0`, `signature changes=0`); format, lint, typecheck, architecture, root/API/Web builds, and diff checks pass. Pinned OnlineCodeEditor `09877bf` passes 56/56 tests and build without revision change. Managed runtime/Doctor and installed-Chrome Playwright passed from exact commit `fbc1e91`, including persisted Problem/favorite, Team/Assignment/Homework, Discussion, Contest, explicit unavailable states, Judge denial, and logout; no page or unexpected HTTP error passed. Manual visual acceptance remains PENDING USER. No migration, deployment, Judge Protocol, production data, or untrusted-code execution boundary changed. See `Docs/FULLSTACK_SYNC_MATRIX.md` and `Docs/reports/OJPLATFORM_FULL_STACK_SYNCHRONIZATION_V1_REPORT.md`.

Windows Dev Runtime Infrastructure Readiness V1 (2026-09-21): PASS for the Windows/local development runtime; no production impact. Fixed the cold Docker/WSL infrastructure startup race in which `scripts/dev-runtime.ps1 start` reported infrastructure success and the Judge DB bootstrap then failed with `Connection terminated unexpectedly` from `pg/lib/client.js` (`dev-runtime.ps1:220` was only the generic command wrapper). Proven root cause: a **false READY**. The runtime accepted container existence, container-level healthcheck, attached network/port mapping, and an accepted TCP connection (`Test-TcpPort`) as readiness; the container healthcheck runs inside the container and never observes the published path. On the affected host the Compose bridge `ojplatform-local` was `172.18.0.0/16` while the WSL2 host-NAT `eth0` was `172.18.0.1/30`, so postgres `172.18.0.2` and redis `172.18.0.3` were routed through `eth0` (or as a broadcast address) instead of `br-*`: the Docker port forwarder accepted Windows connections and closed them with no PostgreSQL handshake (176 consecutive failures over 172s), Windows Redis `PING` returned no `+PONG`, while MinIO `172.18.0.4` answered HTTP 200 and `docker exec pg_isready` reported `accepting connections`. Container-only `--force-recreate` kept the colliding IP and did not recover the path; recreating the Compose network (without `-v`) re-allocated `172.19.0.0/16` and restored all three published paths. Fix: published-path protocol readiness probes (PostgreSQL `connect + SELECT 1` on 55432, Redis `PING -> +PONG` on 56379, MinIO `/minio/health/ready` HTTP 200 on 59000), a bounded `InfraReadyTimeoutSec=120` / `InfraReadyPollSec=2` wait printing `STARTING` / `READY (n s)` / `FAILED`, transient-vs-configuration failure classification that never retries credentials/authentication/missing database/SQL failures, one bounded network-level reconcile (`down --remove-orphans` + `up -d` without `-v`, with `ojplatform-postgres-data` presence asserted before and after), complete failure diagnostics (container state, health, restart count, exit code, publishing, host TCP, probe result, container IP, WSL route, network subnet, log tail, reproduce commands), published-path reporting in `status`, and a defensive bounded transient-only retry with 1s/2s/4s/8s backoff in `scripts/judge-service-bootstrap.mjs`. Runtime evidence on the physical Windows 11 + WSL2 host: cold start from containers `exited` PASS (`postgres READY (12.2s)`, `redis READY (12.2s)`, `minio READY (17.7s)`, `Infrastructure START READY (17.7s)`, bootstrap READY, migrations PASS, worker ONLINE), slow-Postgres test PASS with no premature bootstrap (postgres 11.4s vs redis/minio 6.1s), second start PASS (10.6s, all REUSE), bootstrap retry evidence (transient 5 attempts then FAILED TRANSIENT, admin auth failure and missing database FAILED FATAL on attempt 1 without retry), acceptance-then-close and accept-then-silent probes correctly classified transient/repairable while wrong credentials were classified configuration. Regression: all 8 `scripts/*.ps1` PowerShell 5.1 parse OK, 5 focused PowerShell suites PASS, 4 Vitest files / 38 tests PASS (migration architecture, deployment contract, redis ACL compose, execution-cell Docker boundary), architecture dependency gate PASS, `tsc --noEmit` PASS, Prettier PASS and `git diff --check` clean. Data safety: no `compose down -v`, no volume/DB removal or reset; product DB fingerprint (57 tables, 130 users, 114 problems, 69355 submissions, 19 databases) and all three named volumes unchanged. A pre-existing, unrelated dev-DB blocker was found and repaired with explicit user authorization: the product ledger contained one legacy `0000_platform_metadata.down` row (applied 2026-09-03 by a pre-manifest runner) that the current fixed-manifest runner rightly rejects as `MIGRATION_LEDGER_UNKNOWN_VERSION`; the ledger table was dumped to `%LOCALAPPDATA%\OJPlatform\runtime\ojplatform-local\backup\ledger-20260921-141350.sql` and only that row deleted, after which the pending legitimate migration `0036_contest_development_provenance` applied. `PRODUCTION_DEPLOYMENT = UNCHANGED` (`deploy/` untouched); `DEV_RUNTIME_INFRA_READINESS = PASS`. A later version-mismatch reconcile after committing the fix passed as well (all services restarted from the committed HEAD, `MIXED SOURCE = False`, worker `RUNNING_OWNED`). During validation a dev-only test case passed a wrong `JUDGE_DATABASE_PASSWORD` to the idempotent bootstrap, which applies `ALTER ROLE ... PASSWORD`: the live `oj_judge_service` password was silently changed and the Judge Service returned HTTP 500 `password authentication failed` for `GET /v1/admin/nodes` until the bootstrap was re-run with the Runtime Manager secret (role connection and admin endpoint re-verified 200; no table or row affected, dev test helper corrected). The bootstrap now verifies the Judge role credential as a postcondition before reporting success, and the destructive-`ALTER ROLE` hazard is recorded as a follow-up. See `Docs/reports/OJPLATFORM_DEV_RUNTIME_INFRASTRUCTURE_READINESS_V1_REPORT.md`.
...
Frontend Typography Audit & Unification V1 (2026-09-22): PASS / READY FOR INTEGRATION on `codex/frontend-typography-unification-v1`, based on main `58b7c8b`. A complete Web font inventory found 157 declarations spread across UI Sans, Georgia/宋体, Arial, 楷体 and multiple mono stacks; only 15 tabular-numeral declarations existed. All app typography now resolves through canonical system-native `--font-ui`, `--font-heading`, and `--font-code` tokens; ordinary IDs and UI numbers remain sans while table/rank/time/memory/score/statistics/pagination surfaces use tabular lining numerals; 33 weight-800 declarations were normalized to 700; form controls inherit; OnlineCodeEditor chrome is UI Sans while CodeMirror/source/sample content remains canonical mono. Chrome before/after and computed-style audits passed at 1440, 1024 and effective-200%-width 720 with no document overflow, no visible control clipping, equal tabular digit widths, no new font request, and zero fatal page errors. Typecheck, Web build, architecture, changed lint/format, typography contract 4/4, focused Web regression 59/59, and diff checks pass. `pnpm test:web` remains 11/12 on both unchanged main and feature due the pre-existing submission statistics mock omitting `trend`; no business/API/backend/Judge/deployment contract changed. Main was not merged; manual UI acceptance remains PENDING USER. Report: `Docs/reports/OJPLATFORM_FRONTEND_TYPOGRAPHY_UNIFICATION_V1_REPORT.md`.

Specialized AI Host Sync V1 (2026-09-24): PASS / MERGED. OJPlatform `main` now recognizes, grants, discovers, and dispatches the AI Bridge Stage 7 specialized capability `code.debug.analyze@1.0`, preserving its logical identity end to end instead of rewriting it to the underlying `ai.structured.generate` primitive. Delivered on `feature/ai-specialized-capability-sync-v1` (base `5f9513f`, feature head `381c905`) and integrated with conflict-free `--no-ff` merges `4848116` and `fb42e73`; the merged tree is identical to the qualified feature tree. The capability broker gained optional `kind`/`readiness` on descriptors (maturity kept independent of availability) and an optional opaque `provenance` field on invocations; the AI Bridge adapter now declares the specialized ids the loaded plugin serves (AI Bridge's own manifest declares only its generic primitives, and AI Bridge was not modified), cross-checks `AIBRIDGE_REQUEST_CONTRACT_VERSION = 1.2`, and validates the prompt-provenance pair at the host boundary — `promptApplied: true` requires a valid `promptVersion`, `false` forbids it, omitted stays omitted — rejecting invalid combinations as `INVALID_REQUEST` / `INVALID_PROMPT_PROVENANCE` with zero dispatch. Capability references now accept `@major.minor`, and one permission model covers generic and specialized capabilities with deny-by-default grants from site registration or plugin `consumesCapabilities`. A new host conformance suite (29 tests; 5 AI suites 82/82) plus extended broker/manifest coverage exercises grant, discovery, dispatch, caller-spoof denial, provenance roundtrip and invariants, kill switch, cancellation, error mapping, usage attribution, idempotency key fidelity, and optional-subsystem boot, driven by verbatim vendored AI Bridge Stage 7 fixtures with no cross-repository import and no absolute-path coupling. Validation: `pnpm test:baseline` PASS (1122 passed / 48 known failures / 0 new, `missing 0, newFailures 0, changedFailureSignatures 0, newPending 0`), integration 16 passed / 4 skipped (20 total) exit 0, typecheck/lint/build/architecture gates PASS, browser secret scan clean. Runtime contract changed NO, no new migration (provenance persistence already existed in `0038`), and `deploy`/compose/installer were untouched. AI Bridge and AlgoQuest repositories were read-only and unmodified; real AlgoQuest AI end-to-end remains NOT RUN. AI Bridge Stage 7 host qualification = PASS. Report: `Docs/reports/OJPLATFORM_SPECIALIZED_AI_HOST_SYNC_V1_REPORT.md`.

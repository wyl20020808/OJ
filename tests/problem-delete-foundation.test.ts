...
const input = { slug: 'delete-me', title: 'Delete me', background: '', statement: 'x', inputDescription: 'x', outputDescription: 'x', constraints: 'x', notes: '', timeLimitMs: 1000, memoryLimitBytes: 1024, visibility: 'private' as const, status: 'draft' as const, difficulty: null, testdataVersion: null, samples: [], examples: [], tags: [] };
...
describe('problem deletion foundation', () => {
  it('tombstones, filters, preserves history, and is idempotent', async () => {
    ...
    const policy = { can: async (action: string) => action === 'delete' || action === 'read' };
    const service = new ProblemService(repository, policy);
    const created = await repository.create({ ...input, authorId: 'author' });
    const deleted = await service.delete(created.id, { reason: 'fixture cleanup', expectedUpdatedAt: created.updatedAt }, context);
    ...
    await expect(service.detail(created.id, context)).rejects.toThrow('Problem not found');
    expect(await service.delete(created.id, { reason: 'repeat', expectedUpdatedAt: 'stale' }, context)).toEqual(deleted);
  ...
  it('rejects stale optimistic delete', async () => {
    ...
    const created = await repository.create({ ...input, authorId: 'author' });
    await expect(service.delete(created.id, { reason: 'x', expectedUpdatedAt: 'stale' }, context)).rejects.toThrow('stale');
  });
});

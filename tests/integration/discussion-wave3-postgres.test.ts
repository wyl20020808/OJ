...
describe.skipIf(!databaseUrl)('Discussion Wave 3 PostgreSQL qualification', () => {
    ...
    try {
      ...
      for (const [n, id] of users.entries()) {
        await client.query(
          ...
          [id, `wave3-${n}-${id.slice(0, 8)}`, `wave3-${n}-${id}@example.test`],
        );
      }
      const repo = new PostgresDiscussionRepository(client);
      const postA = await repo.create({
        authorId: users[0], type: 'ARTICLE', title: 'A', contentMarkdown: 'A', status: 'PUBLISHED',
      });
      const postB = await repo.create({
        authorId: users[0], type: 'ARTICLE', title: 'B', contentMarkdown: 'B', status: 'PUBLISHED',
      });
      const parent = await repo.createComment({ postId: postA.id, authorId: users[0], contentMarkdown: 'parent' });
      const reply = await repo.createComment({ postId: postA.id, authorId: users[1], contentMarkdown: 'reply', parentCommentId: parent.id });
      const nested = await repo.createComment({ postId: postA.id, authorId: users[2], contentMarkdown: 'nested', parentCommentId: reply.id });
      expect(reply.parentCommentId).toBe(parent.id);
      expect(nested.parentCommentId).toBe(reply.id);
      const other = await repo.createComment({ postId: postB.id, authorId: users[0], contentMarkdown: 'other' });
      await expect(repo.createComment({ postId: postA.id, authorId: users[1], contentMarkdown: 'cross', parentCommentId: other.id })).rejects.toThrow('Parent comment not found');
      await repo.tombstoneComment(parent.id);
      const visible = await repo.listComments(postA.id, undefined, 20, users[1]);
      ...
      expect((await repo.getComment(reply.id, users[0]))?.viewerLiked).toBe(true);

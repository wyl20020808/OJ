...
describe('ProblemStatementRenderer security contract', () => {
  it('renders canonical sections once, with samples between output and constraints', () => {
    ...
    const sections = [...container.querySelectorAll('[data-section]')].map(
    ...
    expect(container.querySelectorAll('[data-section="notes"]')).toHaveLength(
      1,
    );
  ...
  it('omits empty optional sections and keeps sample pairs grouped', () => {
    ...
    expect(
      container.querySelector('[aria-label="样例 2 输入"]'),
    ).toHaveTextContent('c');
    expect(
      container.querySelector('[aria-label="样例 2 输出"]'),
    ).toHaveTextContent('d');
  });
});

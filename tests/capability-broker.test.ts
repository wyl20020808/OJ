...
function makeFakeAiProvider(overrides: Partial<{ status: 'AVAILABLE' | 'DEGRADED' | 'UNAVAILABLE' | 'DISABLED' }> = {}) {
  const calls: FakeCall[] = [];
  const provider: CapabilityProvider = {
    providerId: 'ai.bridge',
    manifest: fakeAiManifest,
    listCapabilities: () => [
      { id: 'ai.text.generate', version: '1.0', status: overrides.status ?? 'AVAILABLE' },
      { id: 'ai.structured.generate', version: '1.0', status: overrides.status ?? 'AVAILABLE' },
    ],
    execute: async (capability, version, invocation, caller) => {
      ...
      const result: CapabilityResult = {
        status: 'SUCCEEDED',
        output: capability === 'ai.text.generate' ? { text: 'fake', finishReason: 'STOP' } : { summary: 'fake' },
...
function makeBroker() {
  ...
  broker.registerSiteCaller({ siteId: 'problem', capabilities: ['ai.text.generate@1.x', 'ai.structured.generate@1.x'] });
...
describe('trusted caller identity', () => {
  it('composes collision-free caller keys for the two namespaces', () => {
    expect(callerKey({ callerType: 'PLUGIN', callerId: 'algoquest.learning-quests' })).toBe('plugin.algoquest.learning-quests');
    expect(callerKey({ callerType: 'SITE', callerId: 'problem' })).toBe('site.problem');
  ...
  it('binds plugin identity from the manifest, not from any request', () => {
    ...
    const client = broker.forPlugin(pluginManifest('test.consumer', ['ai.text.generate@1.x']));
    expect(client?.callerKey).toBe('plugin.test.consumer');
    void client?.execute('ai.text.generate', '1.0', { idempotencyKey: 'turn-1', input: {} });
  ...
  it('makes cross-namespace and cross-plugin spoofing impossible by construction', () => {
    const { broker, calls } = makeBroker();
    const pluginA = broker.forPlugin(pluginManifest('plugin-a', ['ai.text.generate']))!;
    ...
    void pluginA.execute('ai.text.generate', '1.0', { idempotencyKey: 'a-1', input: {} });
    void siteProblem.execute('ai.text.generate', '1.0', { idempotencyKey: 's-1', input: {} });
...
describe('permission model', () => {
  it('allows a plugin that declared the capability', async () => {
    ...
    const client = broker.forPlugin(pluginManifest('consumer-a', ['ai.structured.generate@1.x']))!;
    const result = await client.execute('ai.structured.generate', '1.0', { idempotencyKey: 'p-1', input: {} });
  ...
  it('denies a plugin that never declared AI capabilities', async () => {
    ...
    const result = await client.execute('ai.text.generate', '1.0', { idempotencyKey: 'b-1', input: {} });
  ...
  it('allows the registered site problem module', async () => {
    ...
    const result = await client.execute('ai.text.generate', '1.0', { idempotencyKey: 'sp-1', input: {} });
    expect(result.status).toBe('SUCCEEDED');
  });
  ...
  it('denies a registered site module without the capability and rejects unregistered modules', async () => {
    ...
    const result = await admin.execute('ai.text.generate', '1.0', { idempotencyKey: 'sa-1', input: {} });
    expect(result.status).toBe('FAILED');
    expect(result.error?.code).toBe('PERMISSION_DENIED');
  ...
  it('deny-by-default grant matching', () => {
    expect(isCapabilityGranted(['ai.text.generate'], 'ai.text.generate')).toBe(true);
    expect(isCapabilityGranted(['ai.text.generate@1'], 'ai.text.generate', '1.0')).toBe(true);
    expect(isCapabilityGranted(['ai.text.generate@1.x'], 'ai.text.generate', '1.2')).toBe(true);
    expect(isCapabilityGranted(['ai.text.generate@2'], 'ai.text.generate', '1.0')).toBe(false);
...
describe('availability and kill switch', () => {
  it('reports provider availability with discovery', () => {
    ...
    expect(broker.getCapabilityStatus('no.such.capability')).toBe('UNAVAILABLE');
  ...
  it('global kill switch reports DISABLED and blocks execution', async () => {
    ...
    const client = broker.forSite('problem')!;
    const result = await client.execute('ai.text.generate', '1.0', { idempotencyKey: 'k-1', input: {} });
    expect(result.status).toBe('FAILED');
  ...
  it('provider and capability kill switches block selectively', async () => {
    const { broker } = makeBroker();
    broker.setKillSwitch({ global: false, providers: ['ai.bridge'], capabilities: [] });
    expect(broker.getCapabilityStatus('ai.text.generate')).toBe('DISABLED');
    broker.setKillSwitch({ global: false, providers: [], capabilities: ['ai.structured.generate'] });
    expect(broker.getCapabilityStatus('ai.structured.generate')).toBe('DISABLED');
    expect(broker.getCapabilityStatus('ai.text.generate')).toBe('AVAILABLE');
  });
  ...
  it('rejects duplicate capability registrations and undeclared listings', () => {
    ...
    const second = { ...makeFakeAiProvider().provider, providerId: 'ai.bridge-duplicate' };
    expect(() => broker.registerProvider(second)).toThrow(/already provided/);
    const fake = makeFakeAiProvider();
    const liar = { ...fake.provider, providerId: 'liar', manifest: { ...fake.provider.manifest, providesCapabilities: [] } };
    expect(() => broker.registerProvider(liar)).toThrow(/does not declare/);
    expect(() => broker.registerProvider(first.provider)).toThrow(/already registered/);
  });
});
...
describe('cross-caller isolation and subject binding', () => {
  it('attributes executions to the exact caller and subject', async () => {
    ...
    const plugin = broker.forPlugin(pluginManifest('consumer-a', ['ai.text.generate']))!.withSubject('ojs1_user-a');
    await site.execute('ai.text.generate', '1.0', { idempotencyKey: 'x-1', input: {} });
    await plugin.execute('ai.text.generate', '1.0', { idempotencyKey: 'x-2', input: {} });
...
describe('timeout clamping (host deadline)', () => {
  it('clamps budgets down but never up', async () => {
    ...
    const narrow = { idempotencyKey: 't-2', input: {}, timeoutBudget: { totalMs: 5_000, providerMs: 60_000 } };
...
describe('opaque subject tokens', () => {
  it('mints stable, opaque, namespace-separated tokens', () => {
    ...
    const a2 = createSubjectTokenMinter({ secret: 'test-secret' }).mintForUser('user-uuid-1');
    ...
    expect(Buffer.from(a1.slice(5), 'base64url').toString('utf8')).not.toContain('user-uuid');
  });
  ...
  it('different secrets yield different tokens (no cross-deployment replay)', () => {
    const one = createSubjectTokenMinter({ secret: 'secret-one' }).mintForUser('user-1');
    const two = createSubjectTokenMinter({ secret: 'secret-two' }).mintForUser('user-1');
